    ...
    <v-container class="mainWrap">
        ...
        >
            <v-card-text class="notificationText"
                >プロフィールの登録が完了していません。<br />設定ページより登録お願いします。</v-card-text
            >
            <v-card-actions>
                <v-btn text color="#bc8f8f" :to="{ name: 'UserSetting' }"
                    >プロフィール登録ページへ移動</v-btn
                >
            </v-card-actions>
        </v-card>
        <v-tabs
        ...
        <v-tabs-items v-model="tabConfigurations.tab">
            <v-tab-item value="lady">
                ...
                >
                        ...
                        <v-list-item-content>
                            ...
                            <v-list-item-subtitle>
                                <span class="genderSpan">{{
                                    gender[ladysPost.gender]
                                }}</span>
                                <span v-if="ladysPost.age"
                                    >{{ ladysPost.age }}歳</span
                                >
                            </v-list-item-subtitle>
                        </v-list-item-content>
                    </v-list-item>
                    ...
                    <v-img
                        v-if="ladysPost.post_image"
                        :src="ladysPost.post_image"
                    ></v-img>
                ...
                >
                    <v-list-item>
                        ...
                        <v-list-item-content>
                            ...
                            <v-list-item-subtitle>
                                <span class="genderSpan">{{
                                    gender[mensPost.gender]
                                }}</span>
                                <span v-if="mensPost.age"
                                    >{{ mensPost.age }}歳</span
                                >
                            </v-list-item-subtitle>
                        </v-list-item-content>
                    ...
                    <v-img
                        v-if="mensPost.post_image"
                        :src="mensPost.post_image"
                    ></v-img>
                    <v-card-actions class="iconWrap">
                        <PostBookmarkComponent
                            v-if="
    ...
    methods: {
        ...
        removePostBookmark: function(id) {
            axios
                ...
                .then(res => {
                    const order = this.postBookmarkedId.findIndex(
                        item => item == id
                    );
